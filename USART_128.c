...
void USART_Init( unsigned int baud, uint16_t n )
{
	sei();
	if(n)
	{
...
void USART_InterruptEnable(uint16_t n)
{
	if(n)
	{
		UCSR1B|=(1<<RXCIE1);
...
void USART_Transmitchar( unsigned char data, uint16_t n )
{
	if(n)
	{
...
unsigned char USART_Receive(uint16_t n )
{
	if(n)
	{
		while (! (UCSR1A & (1 << RXC1)) );
...
void USART_TransmitString(char *str, uint16_t n)
{
	 while(*str>0)
	 {
...
void USART_TransmitNumber(long int num, uint16_t n)
{
	if(num<0)
	{
